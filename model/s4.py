...
def kernel_DPLR(Lambda, P, B, C, step, L):
    Omega_L = torch.exp((-2j * math.pi) * (torch.arange(L) / L))
...
class SSM(nn.Module):
    def __init__(self, dim, N):
        ...
        self.step = log_step_initializer(shape=(1,))
        print(init_Lambda.shape)
        
        self.x = torch.zeros(dim,N,1, dtype=torch.cfloat)
